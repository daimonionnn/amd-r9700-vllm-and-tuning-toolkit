...
> one (Samsung/F40) had a thermal-interface defect that throttled it, and
> under TP=2 it gated the healthy card. The repo's ~1841–1894 t/s TP=2 prefill is
> therefore a **lower bound**, not the ceiling for a healthy pair. (That card has
> since been fixed — the benchmark numbers still date from when it wasn't.)

## Index

| Document | What it covers |
|:----|:----|
| [r9700-mem-vendor-bios-variance.md](r9700-mem-vendor-bios-variance.md) | The two "identical" R9700s differ in GDDR6 vendor (Hynix vs Samsung) and VBIOS (F50 vs F40), and tune differently. Includes the **thermal investigation** that traced card2's ~7 % prefill/power deficit to a die-to-heatsink defect, not weaker silicon — plus the **post-RMA retest** confirming that defect is now gone. |
